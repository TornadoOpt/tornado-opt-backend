...
impl State {
    ...
    pub fn tick(&mut self, commitment: Fr) -> anyhow::Result<()> {
        ...
        self.hash_chain_root = B256::from_slice(&new_hash_chain_root[..31]);

        // add to merkle tree
        let index = self.commitments.len();
        let old_root = self.merkle_tree.get_root();
        let proof = self.merkle_tree.prove(index);
        self.merkle_tree.update_leaf(index, commitment);
        let new_root = self.merkle_tree.get_root();
