...
fn get_function_selector(mode: NovaVerificationMode, state_len: usize) -> [u8; 4] {
    let fn_sig = match mode {
        NovaVerificationMode::Explicit => format!(
        ...
        NovaVerificationMode::OpaqueWithInputs => format!(
            "verifyOpaqueNovaProofWithInputs(uint256,uint256[{state_len}],uint256[{state_len}],uint256[25])"
        ),
    };
